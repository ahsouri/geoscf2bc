...
def run_bcon(date1):
    bcpaths = default(
      ...
      SDATE=f'{date1}T00', EDATE=f'{date1}T00',m3path ='/discover/nobackup/asouri/MODELS/CMAQv5.5/data/mcip/CONUS_8km/METCRO3D_CONUS_8km_20230530.nc'
      )
    return None
...
out = Parallel(n_jobs=12,verbose=10)(delayed(run_bcon)(
           datarange[k].strftime('%Y%m%d')) for k in range(len(datarange)))
